...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < romanNumerals.length; i += 1) {
    while (num2 >= romanNumerals[i].value) {
      result += romanNumerals[i].numeral;
      num2 -= romanNumerals[i].value;
    }
  }
  return result;
}
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length / 2; i += 1) {
    for (let j = i; j < matrix.length - i - 1; j += 1) {
      ...
      result[i][j] = result[matrix.length - j - 1][i];
      result[matrix.length - j - 1][i] =
        result[matrix.length - i - 1][matrix.length - j - 1];
      result[matrix.length - i - 1][matrix.length - j - 1] =
        result[j][matrix.length - i - 1];
      result[j][matrix.length - i - 1] = temp;
    }
  }
  return result;
}
...
function getNearestBigger(number) {
  const arrNum = Array.from(`${number}`, (item) => Number(item));
  ...
  for (let i = arrNum.length - 1; i >= 0; i -= 1) {
    if (arrNum[i - 1] < arrNum[i]) {
      ...
      for (let j = 0; j < arrNum.length; j += 1)
        if (arrNum[j] === min) indexOfMinRight = j;
      const tmp = arrNum[indexOfMinRight];
      arrNum[indexOfMinRight] = arrNum[i - 1];
      arrNum[i - 1] = tmp;
